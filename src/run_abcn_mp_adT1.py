...
def evaluate(args, model, eval_dataset): 
    ...
    for batch in eval_dataloader:
        ...
        loss = dae_loss + decoder_loss + decoder_fuse_loss + bottlneck_loss + mlm_loss + bc_loss + cl_loss + sem_loss
...
def main_worker(local_rank, nprocs):
    ...
    if common_config['MODEL'] == 'TRAIN':
        ...
        if local_rank == 0:
            ...
            args.checkpoint_dir = os.path.join(os.path.join(base_path,'model/abcn_mp','pretrain_880w_'+time.strftime('%Y%m%d',time.localtime())))
            ...
            writer = SummaryWriter(log_dir=os.path.join(base_path,'log/abcn_mp'),filename_suffix='_880w')
