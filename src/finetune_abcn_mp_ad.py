...
from self_attention.dataset import AdAppMaskDatasetABC, AdAppMaskDatasetABC_finetune
...
def train(local_rank, args, model, train_dataset, eval_dataset=None, writer=None):
    ...
    for idx in range(args.train.start_epoch, args.train.epoch):
        ...
        for bid, batch in enumerate(train_dataloader):
            keep_app_ids, app_ids, type_ids, time_ids, cate_ids, mask_ids, mask_label_ids, bc_app_ids, bc_masks, bc_label_dist, aids, labels = [x.to(args.device) for x in batch]
            model.train() 
            pooled_out, bc_emb, fusion_emb, dae_loss, decoder_loss, decoder_fuse_loss, bottlneck_loss, mlm_loss, bc_loss, cl_loss, y_pred, down_loss = model(keep_app_ids, app_ids, type_ids, time_ids, cate_ids, mask_ids, mask_label_ids, bc_app_ids, bc_masks, bc_label_dist, aids, labels)
            loss = 0.1 * (dae_loss + decoder_loss + decoder_fuse_loss + bottlneck_loss + mlm_loss + bc_loss + cl_loss) + 0.1 * down_loss
            ...
            if global_step % args.train.logging_step==0:
                if local_rank == 0: # 多gpu，选择其中一个看效果就好
                    ...
                    print("step={:d}, dae_loss={:.4f},neck_loss={:.4f},decoder_loss={:.4f},f_decoder_loss={:.4f},mlm_loss={:.4f},bc_loss={:.4f}, cl_loss={:.4f}, down_loss={:.4f}, loss={:.4f}, avg_ppl={:.4f}".format(
                                    global_step, dae_loss.item(),bottlneck_loss.item(),decoder_loss.item(), decoder_fuse_loss.item(),mlm_loss.item(), bc_loss.item(), cl_loss.item(), down_loss.item(), loss.item(), avg_ppl))
...
def evaluate(args, model, eval_dataset): 
    ...
    eval_loss = 0.0
    tdae_loss = tbottlneck_loss = tdecoder_loss = tdecoder_fuse_loss = tmlm_loss = tbc_loss = tcl_loss = tdown_loss = 0.0
    eval_steps = 0 
    ...
    for batch in eval_dataloader:
        keep_app_ids, app_ids, type_ids, time_ids, cate_ids, mask_ids, mask_label_ids, bc_app_ids, bc_masks, bc_label_dist, aids, labels = [x.to(args.device) for x in batch]
        pooled_out, bc_emb, fusion_emb, dae_loss, decoder_loss, decoder_fuse_loss, bottlneck_loss, mlm_loss, bc_loss, cl_loss, y_pred, down_loss = model(keep_app_ids, app_ids, type_ids, time_ids, cate_ids, mask_ids, mask_label_ids, bc_app_ids, bc_masks, bc_label_dist, aids, labels)
        loss = (dae_loss + decoder_loss + decoder_fuse_loss + bottlneck_loss + mlm_loss + bc_loss + cl_loss) + 0.1 * down_loss
    ...
    tcl_loss /= eval_steps
    tdown_loss /= eval_steps
    perplexity = np.exp(eval_loss)
    result = {'eval_loss': eval_loss, 'dae_loss':tdae_loss,'neck_loss':tbottlneck_loss,
             'decoder_loss':tdecoder_loss, 'decoder_fuse_loss':tdecoder_fuse_loss,'mlm_loss':tmlm_loss, 'bc_loss':tbc_loss, 'cl_loss':tcl_loss, 'down_loss':tdown_loss, 'ppl': perplexity}
...
def main_worker(local_rank, nprocs):
    ...
    # train_file = '/home/notebook/data/group/app-series/ad_platform/adT1/abcn_train_attr_{}.csv'.format(local_rank)
    # val_file = '/home/notebook/data/group/app-series/ad_platform/adT1/abcn_test_attr_{}.csv'.format(local_rank)
    hist_file = '/home/notebook/data/group/app-series/ad_platform/adT1/pretrain_all.csv'
    down_file = "/home/notebook/data/personal/ad/_ad/train.csv"

    hist_df = pd.read_csv(hist_file, sep='\t', dtype=str, nrows=None)
    # hist_df.drop(columns=['label'], inplace=True)
    down_df = pd.read_csv(down_file, dtype=str, nrows=None)
    down_df = down_df.sample(frac=0.1, replace=False, weights=None, random_state=42, axis=None)
    df = hist_df.merge(down_df, how='inner', on='imei')
    df.fillna("unk", inplace=True)
    whole_len = len(df)
    part_len = whole_len // nprocs
    df = df.iloc[local_rank * part_len : (local_rank+1) * part_len]
    print(local_rank, part_len, len(df))
    hist_df = []
    down_df = []
    ...
    vocab_dic = json.load(open("/home/notebook/data/personal/mete/app_iaa_vocab_with_cate.json",'r')) # : app-id table

    with open('/home/notebook/code/personal/appseq/finetune_abcn.yaml','r') as fp:
        cfg = yaml.load(fp)
    ...
    if common_config['MODEL'] == 'TRAIN':
        ...
        if local_rank == 0:
            ...
            args.checkpoint_dir = os.path.join(os.path.join(base_path,'model/abcn_mp','adT1_finetune_p'+time.strftime('%Y%m%d',time.localtime())))
            ...
            writer = SummaryWriter(log_dir=os.path.join(base_path,'log/abcn_mp'),filename_suffix='_adT1')
            atexit.register(writer.close)


        train_dataset = AdAppMaskDatasetABC_finetune(df_train, vocab_dic, common_config)
        eval_dataset = AdAppMaskDatasetABC_finetune(df_eval,vocab_dic, common_config)
        model = torch.load(os.path.join(base_path,'model/abcn_mp/pretrain_880w_20230217_17.3293/checkpoint_last/model.pt'), map_location = torch.device(args.device))
