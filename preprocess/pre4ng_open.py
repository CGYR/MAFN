...
def filter_time(file, tgt_file):
    # only remain interaction in [20230215, 20230228]
    out_imei_list = []
    out_pack_list = []
    out_time_list = []
...
def merge_sp_ae_bc(ae_file, bc_files, tgt_path, sp_num):
    ...
    df = pd.merge(df_ae, df_bc, on="imei", how="inner")
...
def merge_sp_ae_bc_big(ae_file, bc_files, tgt_path, sp_num):
    # merge aetn & bc pretrain files, and split to n part for multi-gpu
    df_ae = pd.read_csv(ae_file, sep='\t', dtype=str)
    df = pd.DataFrame()
    for file in bc_files:
        df_bc = pd.read_csv(file, sep='\t', usecols=["imei","pack_name"], dtype=str)
        df_bc.rename(columns={"pack_name":"open_seq"}, inplace=True)
        df_tmp = pd.merge(df_ae, df_bc, on="imei", how="inner")
...
def merge_train_files(emb_files, tgt_file):
    ...
    for file in tqdm(emb_files):
        ...
        df = pd.concat([df, df_tmp])
    print("ng imei num:{}".format(len(df)))
    df.to_csv(tgt_file, index=False)


def merge_ng_embs(ae_files, sum_files, tgt_file):
    # cat ae & sum emb files, and merge to one file
    df_ae = pd.DataFrame()
    for file in tqdm(ae_files):
        print(file)
        df_tmp = pd.read_csv(file, dtype=str)
        df_tmp.drop_duplicates(subset=["imei"], inplace=True)
        df_ae = pd.concat([df_ae, df_tmp])
    df_sum = pd.DataFrame()
    for file in tqdm(sum_files):
        print(file)
        df_tmp = pd.read_csv(file, dtype=str)
        df_tmp.drop_duplicates(subset=["imei"], inplace=True)
        df_tmp = df_tmp.rename(columns={str(i) : "e_" + str(i) for i in range(64)})
        df_sum = pd.concat([df_sum, df_tmp])
    df = pd.merge(df_ae, df_sum, on="imei")
    assert len(df) == len(df_ae)
    df.to_csv(tgt_file, index=False)
    
if __name__=='__main__':
    data_path = "/home/notebook/data/group/app-series/pretrain_newgame/test_open/"
    ...
    # print("fileter time ok!")

    # for i in range(len(open_files)):
    #     in_file = open_files[i]
    #     out_file = data_path + "bc_{}.csv".format(i)
    #     print(out_file)
    #     filter_users(in_file, out_file)

    # print("filter apps ok!")
    ...
    # ae_file = "/home/notebook/data/group/app-series/pretrain_newgame/train/un_in_clean.csv"
    # merge_sp_ae_bc(ae_file, open_files, data_path, 4)
    # merge_sp_ae_bc_big(ae_file, open_files, data_path, 4)

    # NOTE repeat process, for 0301~0307
    # open_files = glob.glob(data_path + "shixu__20230320_imei_hpjy_train_2023*")
    ...
    # merge train embs
    train_sum_files = ["/home/notebook/data/group/app-series/pretrain_newgame/train_open/imei_embs_sum_last_{}.csv".format(i) for i in range(4)]
    test_sum_files = ["/home/notebook/data/group/app-series/pretrain_newgame/test_open/imei_embs_sum_last_{}.csv".format(i) for i in range(4)]

    merge_train_files(train_sum_files, "/home/notebook/data/group/app-series/pretrain_newgame/train_open/imei_embs_sum_last_train.csv")
    merge_train_files(test_sum_files, "/home/notebook/data/group/app-series/pretrain_newgame/test_open/imei_embs_sum_last_forecast.csv")
    # merge_train_files(train_ae_files, "/home/notebook/data/group/app-series/pretrain_game/hpjy_open/imei_embs_ae_last_train.csv")

    ae_files = glob.glob(data_path + "imei_embs_ae_last_*")
    sum_files = glob.glob(data_path + "imei_embs_sum_last_*")
    # tgt_file = data_path + "imei_embs_cat.csv"

    # print(ae_files)
    # merge_ng_embs(ae_files, sum_files, tgt_file)
