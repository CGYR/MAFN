...
n_gpu=2
n_cpu=10
task="gen abcn user embs for ad `date "+%Y-%m-%d %X"`"
...
# -m 10240
falcon submit   -g "${n_gpu}" -c "${n_cpu}" -n "${task}" -m 10240 -r /home/notebook/code/personal/appseq/src/gen_abcn_mp_ad_sample_embs.py 
